...
//NB : AddGlobalTimer should not be called from an other thread than the main one. Timers will behave strangely in multithreaded applications...
...
//passing pointer on callback object ( no auto_ptr because no sink here )
...
unsigned int Timer<TClass>::callback(unsigned int interval, void* args)
{
    ...
    {
        throw std::logic_error("Error in template <class TClass> Timer<TClass>::callback(), " + (std::string)exc.what());
    }
    catch (...)
    {
        throw std::logic_error("Unhandled Error in template <class TClass> Timer<TClass>::callback()");
    }
}
...
void Timer<TClass>::setCallback(TClass* instance,unsigned int (TClass::*func) (unsigned int, void*), void* args)
{
    ...
    {
        throw std::logic_error("Error in template <class TClass> Timer<TClass>::setCallback(), " + (std::string)exc.what());
    }
    catch (...)
    {
        throw std::logic_error("Unhandled Error in template <class TClass> Timer<TClass>::setCallback()");
    }
}
...
Timer<TClass>::Timer() : _cbargs(0), _timerid()
{
    ...
    {
        throw std::logic_error("Error in template <class TClass> Timer<TClass> Constructor, " + (std::string)exc.what());
    }
    catch (...)
    {
        throw std::logic_error("Unhandled Error in template <class TClass> Timer<TClass> Constructor");
    }
}
...
bool Timer<TClass>::launch(unsigned int interval)
{
    ...
    {
        throw std::logic_error("Error in template <class TClass> Timer<TClass>::launch(), " + (std::string)exc.what());
    }
    catch (...)
    {
        throw std::logic_error("Unhandled Error in template <class TClass> Timer<TClass>::launch()");
    }
}
...
bool Timer<TClass>::abort()
{
    ...
    {
        throw std::logic_error("Error in template <class TClass> Timer<TClass>::abort(), " + (std::string)exc.what());
    }
    catch (...)
    {
        throw std::logic_error("Unhandled Error in template <class TClass> Timer<TClass>::abort()");
    }
}
*/
}
