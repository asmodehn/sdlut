...
namespace RAGE
{
	namespace SDL
	{
		...
		try : _hwspec(frequency, static_cast<Uint8>(channels), samples, MixerImpl::callback )
		{
#ifdef DEBUG
...
int MixerImpl::freeChannel(int index)
{
	...
	void MixerImpl::PauseAll(void) { SDL_PauseAudio(1); }
	void MixerImpl::PlayAll(void) { SDL_PauseAudio(0); }
