...
try : SoundIf(filename, loop_status), pvm_chunk(Mix_LoadWAV_RW(pvm_OriginalData->get_pSDL(),0))
{
	if ( !pvm_chunk.get() )
	{
...
try : SoundIf(s), pvm_chunk(Mix_LoadWAV_RW(pvm_OriginalData->get_pSDL(),0))
{
	if ( ! pvm_chunk.get() )
	{
