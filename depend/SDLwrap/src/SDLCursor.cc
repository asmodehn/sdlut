...
//set Cursor from XPM
SDL_Cursor * SDLCursor::init_system_cursor(const char *image[])
{
  int i, row, col;
  Uint8 data[4*32];
  Uint8 mask[4*32];
...
SDLCursor::SDLCursor(SDLCursorShape sh) throw (std::logic_error)
...
{
	LIB_ERROR("Exception catched on SDLCursor Constructor !!!");
	LIB_ERROR(e.what());
	SDLERROR;
}
